...
class VisualSpace(InputSpace):
    ...
    def __init__(self, params):
        InputSpace.__init__(
            self, params
        )  # InputSpace requires only the update interval
...
class VisualRegion(object):
    ...
    def __eq__(self, other):
        return (
            self.location_x == other.location_x
    ...
    def describe(self):
        s = (
            """Region of visual space centred at (%(location_x),%(location_y)) s of size (%(size_x),%(size_y))s.
               Edges: left=%(left)g, right=%(right)g, top=%(top)g, bottom=%(bottom)g"""
            % self.__dict__
        )
        return s
