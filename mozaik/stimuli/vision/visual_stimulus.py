...
class VisualStimulus(BaseStimulus):
    ...
    def display(self, region, pixel_size):
        ...
        if region.overlaps(self.region):
            if region not in self.region_cache:
                ...
                img_relative_left = (
                    intersection.left - self.region.left
                ) / self.region.width
                img_relative_width = intersection.width / self.region.width
                img_relative_top = (
                    intersection.top - self.region.bottom
                ) / self.region.height
                ...
                img_pixel_size = (
                    xy2ij((self.region.size_x, self.region.size_y)) / self.img.shape
                )  # is self.size a tuple or an array?
                ...
                i_start = img.shape[0] - numpy.round(
                    img_relative_top * img.shape[0]
                ).astype(int)
                delta_i = numpy.round(img_relative_height * img.shape[0]).astype(int)

                l_start = numpy.round(view_relative_left * size_in_pixels[1]).astype(
                    int
                )
                delta_l = numpy.round(view_relative_width * size_in_pixels[1]).astype(
                    int
                )
                k_start = size_in_pixels[0] - numpy.round(
                    view_relative_top * size_in_pixels[0]
                ).astype(int)
                delta_k = numpy.round(view_relative_height * size_in_pixels[0]).astype(
                    int
                )
    ...
    def update(self):
        ...
        try:
            ...
            assert self.img.min() >= 0 or self.img.min() == TRANSPARENT, (
                "frame minimum is less than zero: %g" % self.img.min()
            )
            assert self.img.max() <= 2 * self.background_luminance, (
                "frame maximum (%g) is greater than the maximum luminance (%g)"
