    ...
    def _handle_parameters_and_execute_plots(self, parameters, user_parameters, gs):
        ...
        for (k, (pl, gs, p)) in d.items():
            ...
            if isinstance(pl, SimplePlot):
                ...
                raise TypeError(
                    "The subplot object is not of type Plotting or SimplePlot"
                )
    ...
    def plot(self, params=None):
        ...
        if self.plot_file_name:
            ...
            if self.animation_update_functions != []:
                ...
                self.animation.save(
                    Global.root_directory + self.plot_file_name + ".mov",
                    writer="avconv_file",
                    fps=30,
                    bitrate=5000,
        ...
        t2 = time.time()
        logger.warning(
            self.__class__.__name__ + " plotting took: " + str(t2 - t1) + "seconds"
        )
...
class PlotTuningCurve(Plotting):
    ...
    def __init__(
        ...
        frame_duration=0,
        centering_pnv=None,
        spont_level_pnv=None
    ):
        Plotting.__init__(
            self, datastore, parameters, plot_file_name, fig_param, frame_duration
        )
        ...
        assert (
            len(self.parameters.neurons) > 0
        ), "ERROR, empty list of neurons specified"
        ...
        assert not (
            centering_pnv != None and self.parameters.centered == False
        ), "Supplied centering_pnv but did not set centered to True."

        dsvs = queries.partition_analysis_results_by_parameters_query(
            self.datastore, parameter_list=["value_name"], excpt=True
        )

        for dsv in dsvs:
            dsv = queries.param_filter_query(
                dsv, identifier="PerNeuronValue", sheet_name=self.parameters.sheet_name
            )
            assert matching_parametrized_object_params(
                dsv.get_analysis_result(), params=["value_name"]
            )
            ...
            dic = colapse_to_dictionary(
                [z.get_value_by_id(self.parameters.neurons) for z in self.pnvs[-1]],
                st,
            ...
            if self.parameters.centered and centering_pnv == None:
                ...
                assert period != None, (
                    "ERROR: You asked for centering of tuning curves even though the"
                    " domain over which it is measured is not periodic."
                )
                centers = centering_pnv.get_value_by_id(self.parameters.neurons)
                self.max_mean_response_indexes.append(
                    numpy.array(
                        [
                            numpy.argmin(circular_dist(par, centers[i], period))
        ...
        if self.parameters.pool:
            assert all(
                [p[0].value_units == self.pnvs[0][0].value_units for p in self.pnvs]
            ), (
                "You asked to pool tuning curves across different value_names, but the"
                " datastore contains PerNeuronValue datastructures with different units"
            )
    ...
    def subplot(self, subplotspec):
        if self.parameters.mean:
            ...
            return LinePlot(
                function=self._ploter, length=len(self.parameters.neurons)
    ...
    def _ploter(self, idx, gs):
        ...
        for i, (dic, st, pnv) in enumerate(zip(self.tc_dict, self.st, self.pnvs)):
            ...
            if self.parameters.polar:
                assert period != None, (
                    "ERROR: You asked to plot the tuning curve on polar axis even"
                    " though the domain over which it is measured is not periodic."
                )
            ...
            for k in sorted(dic.keys()):
                    ...
                    for j in range(0, len(self.parameters.neurons)):
                        if self.parameters.centered:
                            vv, p = self.center_tc(
                                ...
                                par,
                                period,
                                self.max_mean_response_indexes[i][j]
                            )
                        else:
                            vv = val[:, j]
                            p = par
                        # v = v + vv
                        v.append(vv)
                    error = numpy.std(v, axis=0) / numpy.sqrt(
                        len(self.parameters.neurons)
                    )
                    val = numpy.mean(v, axis=0)
                    # val = v / len(self.parameters.neurons)
                    par = p
                elif self.parameters.centered:
                    val, par = self.center_tc(
                        val[:, idx], par, period, self.max_mean_response_indexes[i][idx]
                    )
                else:
                    val = val[:, idx]
                ...
                logger.info(
                    "PLOT TUNING CURVE " + str(type(period)) + " " + str(type(pi))
                )

                # if we have a period of pi or 2*pi
                if (
                    period != None
                    and numpy.isclose(period, pi)
                ...
                for p in varying_parameters(
                    ...
                    l = (
                        ...
                        + str(p)
                        + " : "
                        + str(MozaikParametrized.idd(k).getParamValue(p))
                    )
                labels.append(l)
            ...
            if self.spont_level_pnv != None:
                if not self.parameters.mean:
                    sp_level = self.spont_level_pnv.get_value_by_id(
                        self.parameters.neurons
                    )[idx]
                else:
                    sp_level = numpy.mean(
                        self.spont_level_pnv.get_value_by_id(self.parameters.neurons)
                    )
            ...
            if not self.parameters.pool:
                ...
                params = self.create_params(
                    ...
                    i == (len(self.pnvs) - 1),
                    period,
                    self.parameters.neurons[idx],
                    len(xs),
                    self.parameters.polar,
                    labels,
                    idx
                )
                ...
                plots.append(
                    (
                        "TuningCurve_" + pnv[0].value_name,
                        StandardStyleLinePlot(xs, ys, error=errors, subplot_kw=po),
                        gs[i],
                        params
                    )
                )
        ...
        if self.parameters.pool:
            ...
            if not self.parameters.polar:
                plots.append(
                    (
                        "TuningCurve_Stacked",
                        StandardStyleLinePlot(xs, ys, error=errors),
                        gs,
                        params
                    )
                )
    ...
    def center_tc(self, val, par, period, center_index):
        ...
        q = (
            center_index + len(val) / 2
            if center_index < len(val) / 2
            else center_index - len(val) / 2
        )
        z = par[center_index]
        c = period / 2.0
...
class RasterPlot(Plotting):
    ...
    def subplot(self, subplotspec):
        dsv = queries.param_filter_query(
            self.datastore, sheet_name=self.parameters.sheet_name
        )
        return PerStimulusPlot(dsv, function=self._ploter).make_line_plot(subplotspec)

    def _ploter(self, dsv, gs):
        sp = [
            s.get_spiketrain(self.parameters.neurons)
            for s in sorted(
                dsv.get_segments(),
                key=lambda x: MozaikParametrized.idd(x.annotations["stimulus"]).trial
            )
        ]
        ...
        num_trials = (
            numpy.max(
                [
                    MozaikParametrized.idd(s.annotations["stimulus"]).trial
        ...
        x_ticks = [
            0.0,
            float(sp[0][0].t_stop.rescale(pq.s) / 2),
            float(sp[0][0].t_stop.rescale(pq.s))
        ]
        ...
        if self.parameters.spontaneous:
            spont_sp = [
                s.get_spiketrain(self.parameters.neurons)
                for s in sorted(
                    dsv.get_segments(null=True),
                    key=lambda x: MozaikParametrized.idd(
                        x.annotations["stimulus"]
                    ).trial
                )
            ]
            sp = [
                RasterPlot.concat_spiketrains(sp1, sp2)
                for sp1, sp2 in zip(spont_sp, sp)
            ]
            x_ticks = [
                float(spont_sp[0][0].t_start.rescale(pq.s)),
                0.0,
                float(sp[0][0].t_stop.rescale(pq.s) / 2),
                float(sp[0][0].t_stop.rescale(pq.s))
            ]
        ...
        if self.parameters.trial_averaged_histogram:
            ...
            return [
                (
                    ...
                    SpikeRasterPlot([sp]),
                    gs[:3, 0],
                    {"x_axis": False, "x_label": None}
                ),
                (
                    "SpikeHistogramPlot",
                    SpikeHistogramPlot([sp], num_trials),
                    gs[3, 0],
                    {"x_ticks": x_ticks}
                )
            ]
            ...
            return [
                ("SpikeRasterPlot", SpikeRasterPlot([sp]), gs, {"x_ticks": x_ticks})
            ]
    ...
    def concat_spiketrains(ssp1, ssp2):
        ...
        for sp1, sp2 in zip(ssp1, ssp2):
            ...
            l.append(
                NeoSpikeTrain(
                    numpy.concatenate(
                        (sp1.magnitude - sp1.t_stop.magnitude, sp2.magnitude)
                    ),
                    t_start=-sp1.t_stop,
                    t_stop=sp2.t_stop,
                    units=sp1.units
...
class VmPlot(Plotting):
    ...
    def subplot(self, subplotspec):
        ...
        return PerStimulusPlot(
            dsv, function=self._ploter, title_style="Clever"
        ).make_line_plot(subplotspec)

    def _ploter(self, dsv, gs):
        vms = [
            s.get_vm(self.parameters.neuron)
            for s in sorted(
                dsv.get_segments(),
                key=lambda x: MozaikParametrized.idd(x.annotations["stimulus"]).trial
            )
        ]
        time_axis = numpy.arange(0, len(vms[0]), 1) / float(len(vms[0])) * float(
            vms[0].t_stop
        ) + float(vms[0].t_start)
        ...
        if self.parameters.spontaneous:
            ...
            spont_time_axis = (
                numpy.arange(0, len(spont_vms[0]), 1)
                / float(len(spont_vms[0]))
                * float(spont_vms[0].t_stop)
                - float(spont_vms[0].t_stop)
                + float(vms[0].t_start)
            )
            time_axis = numpy.concatenate((spont_time_axis, time_axis))
            vms1 = [
                numpy.concatenate((svm.magnitude, vm.magnitude))
                for vm, svm in zip(vms, spont_vms)
            ]
        else:
            vms1 = vms
...
class GSynPlot(Plotting):
    ...
    def subplot(self, subplotspec):
        ...
        return PerStimulusPlot(
            dsv, function=self._ploter, title_style="Standard"
        ).make_line_plot(subplotspec)
    ...
    def concat_asl(asl1, asl2):
        ...
        return NeoAnalogSignal(
            numpy.concatenate((asl1.magnitude, asl2.magnitude)),
            t_start=-asl1.t_stop,
    ...
    def _ploter(self, dsv, gs):
        ...
        if self.parameters.spontaneous:
            ...
            gsyn_es = [
                GSynPlot.concat_asl(s, n) for n, s in zip(gsyn_es, spont_gsyn_es)
            ]
            gsyn_is = [
                GSynPlot.concat_asl(s, n) for n, s in zip(gsyn_is, spont_gsyn_is)
            ]
    ...
    def _ploter(self, dsv, subplotspec):
        ...
        if len(list(self.parameters.sheet_activity.keys())) != 0:
            ...
            d.append(
                (
                    "Activity_plot",
                    ActivityMovie(dsv, self.parameters.sheet_activity),
                    gs[0, 0],
                    {}
                )
            )
                        ...
                        ParameterSet(
                            {
                                ...
                                "spontaneous": self.parameters.spontaneous,
...
class AnalogSignalListPlot(Plotting):
    ...
    def _ploter(self, dsv, subplotspec):
        ...
        assert (
            len(self.analog_signal_list) == 1
        ), "Currently only one AnalogSignalList per stimulus can be plotted"
        ...
        params["x_label"] = (
            self.analog_signal_list.x_axis_name
            + "("
            + a.t_start.dimensionality.latex
            + ")"
        )
        ...
        params["mean"] = True
        return [
            ("AnalogSignalPlot", StandardStyleLinePlot(xs, ys), subplotspec, params)
        ]
...
class AnalogSignalPlot(Plotting):
    ...
    def _ploter(self, dsv, subplotspec):
        ...
        assert (
            len(self.analog_signal) == 1
        ), "Currently only one AnalogSignal per stimulus can be plotted"
        ...
        params["x_lim"] = (a.t_start.magnitude, a.t_stop.magnitude)
        params["x_label"] = (
            self.analog_signal.x_axis_name + "(" + a.t_start.dimensionality.latex + ")"
        )
...
class ConductanceSignalListPlot(Plotting):
    ...
    def subplot(self, subplotspec):
        dsv = queries.param_filter_query(
            self.datastore, identifier="ConductanceSignalList"
        )
        return PerStimulusADSPlot(
            dsv, function=self._ploter, title_style="Clever"
        ).make_line_plot(subplotspec)
    ...
    def _ploter(self, dsv, subplotspec):
        ...
        assert (
            len(conductance_signal_list) == 1
        ), "Currently only one ConductanceSignalList per stimulus can be plotted"
...
class PerNeuronPairAnalogSignalListPlot(Plotting):
    ...
    def _ploter(self, dsv, subplotspec):
        ...
        asl = self.analog_signal_list.get_asl_by_id_pair(
            (self.analog_signal_list.ids)[0]
        )
        ...
        params["x_label"] = (
            self.analog_signal_list.x_axis_name
            + "("
            + asl.t_start.dimensionality.latex
            + ")"
        )
        params["y_label"] = self.analog_signal_list.y_axis_name
        ...
        return [
            (
                "AnalogSignalPlot",
                StandardStyleLinePlot([times], [asl]),
                subplotspec,
                params
            )
        ]
...
class RetinalInputMovie(Plotting):
    ...
    def __init__(
        ...
        self.retinal_input = [
            self.retinal_input[i]
            for i in range(0, len(self.st))
            if MozaikParametrized.idd(self.st[i]).name != "InternalStimulus"
        ]
        self.st = [
            self.st[i]
            for i in range(0, len(self.st))
    ...
    def subplot(self, subplotspec):
        return LinePlot(
            function=self._ploter, length=len(self.retinal_input)
        ).make_line_plot(subplotspec)
    ...
    def _ploter(self, idx, gs):
        ...
        return [
            (
                "PixelMovie",
                PixelMovie(
                    self.retinal_input[idx],
                    MozaikParametrized.idd(self.st[idx]).background_luminance
                ),
                gs,
                {"x_axis": False, "y_axis": False, "title": title}
            )
        ]
...
class ActivityMovie(Plotting):
    ...
    def _ploter(self, dsv, gs):
        ...
        if self.parameters.exp_time_constant != 0:
            ...
            exp_kernel = numpy.flip(
                numpy.exp(
                    -(bins[: numpy.int(numpy.floor(3 * etc / bw))] - start) / etc
                ),
                axis=0
            )
        ...
        for spike_trains in sp:
            ...
            for st in spike_trains:
                tmp = numpy.histogram(st.magnitude, bins, (start, stop))[0] / (
                    bw.rescale(pq.s).magnitude
                )
                if self.parameters.exp_time_constant != 0:
        ...
        posx = pos[
            0,
            self.datastore.get_sheet_indexes(
        ...
        if not self.parameters.scatter:
            xi = numpy.linspace(
                numpy.min(posx) * 1.1, numpy.max(posy) * 1.1, self.parameters.resolution
            )
            yi = numpy.linspace(
                numpy.min(posx) * 1.1, numpy.max(posy) * 1.1, self.parameters.resolution
            )
            ...
            return [
                (
                    "PixelMovie",
                    PixelMovie(40000.0 * numpy.array(movie)),
                    gs,
            ...
            return [
                (
                    "ScatterPlot",
                    ScatterPlotMovie(posx, posy, h.T),
                    gs,
                    {"x_axis": False, "y_axis": False, "dot_size": 40}
                )
            ]
...
class PerNeuronValuePlot(Plotting):
    ...
    def __init__(self, datastore, parameters, plot_file_name=None, fig_param=None):
        ...
        self.pnvs = []
        self.sheets = []
        self.dsv = queries.param_filter_query(
            self.datastore, identifier="PerNeuronValue"
        )

    def subplot(self, subplotspec):
        return ADSGridPlot(
            self.dsv,
            function=self._ploter,
            x_axis_parameter="value_name",
            y_axis_parameter="sheet_name"
        ).make_grid_plot(subplotspec)

    def _ploter(self, dsv, gs):
        pnvs = dsv.get_analysis_result(identifier="PerNeuronValue")

        if self.parameters.cortical_view:
            assert len(pnvs) <= 1, logger.error(
                "We can only display single stimulus parametrization in cortical view,"
                " but you have suplied multiple in datastore."
            )
            pnv = pnvs[0]
            ...
            return [
                (
                    "ScatterPlot",
                    ScatterPlot(posx, posy, values, periodic=periodic, period=period),
                    gs,
                    params
                )
            ]
        else:
            assert queries.ads_with_equal_stimulus_type(dsv), logger.error(
                "Warning sheet name and value name does't seem to uniquely identify set"
                " of PerNeuronValue ADS with the same stimulus type"
            )
            params = {}
            params["y_label"] = "# neurons"

            varying_stim_parameters = sorted(
                varying_parameters(
                    ...
                    (
                        ",".join(
                            [
                                ...
                                for p in varying_stim_parameters
                            ]
                        ),
                        pnv
                    )
            ...
            if len(a) > 1:
                return [
                    (
                        "HistogramPlot",
                        HistogramPlot(
                            [z[1].values for z in a], labels=[z[0] for z in a]
                        ),
                        gs,
                        params
                    )
...
class PerNeuronValueScatterPlot(Plotting):
    ...
    def __init__(self, datastore, parameters, plot_file_name=None, fig_param=None):
        ...
        for sheet in datastore.sheets():
            pnvs = datastore.get_analysis_result(
                identifier="PerNeuronValue", sheet_name=sheet
            )
            if len(pnvs) < 2:
            ...
            for i in range(0, len(pnvs)):
                for j in range(i + 1, len(pnvs)):
                    if (
                        pnvs[i].value_units == pnvs[j].value_units
                    ) or not self.parameters.only_matching_units:
                        self.pairs.append((pnvs[i], pnvs[j]))
                        self.sheets.append(sheet)

        assert len(self.pairs) > 0, (
            "Error, not pairs of PerNeuronValue ADS in datastore seem to have the same"
            " value_units"
        )
    ...
    def subplot(self, subplotspec):
        return LinePlot(
            function=self._ploter, length=self.length, shared_axis=False
        ).make_line_plot(subplotspec)
    ...
    def _ploter(self, idx, gs):
        ...
        if pair[0].stimulus_id == None or pair[1].stimulus_id == None:
            ...
            MozaikParametrized.idd(pair[0].stimulus_id).name
            != MozaikParametrized.idd(pair[1].stimulus_id).name
        ):
            p2 = ["name"]
        else:
            p2 = varying_parameters(
                [MozaikParametrized.idd(p.stimulus_id) for p in pair]
            )
        p1 = [x for x in p1 if ((x != "value_name") and (x != "stimulus_id"))]

        x_label = (
            pair[0].value_name + "(" + pair[0].value_units.dimensionality.latex + ")"
        )
        y_label = (
            pair[1].value_name + "(" + pair[1].value_units.dimensionality.latex + ")"
        )
        ...
        for p in p2:
            x_label += (
                ...
                + str(p)
                + " = "
                + str(MozaikParametrized.idd(pair[0].stimulus_id).getParamValue(p))
            )
            y_label += (
                "\n"
                + str(p)
                + " = "
                + str(MozaikParametrized.idd(pair[1].stimulus_id).getParamValue(p))
            )
        ...
        if (
            pair[0].value_units != pair[1].value_units
            or pair[1].value_units == pq.dimensionless
        ):
        ...
        if self.parameters.ignore_nan:
            idxs = numpy.logical_not(
                numpy.logical_or(
                    numpy.isnan(numpy.array(x)), numpy.isnan(numpy.array(y))
                )
            )
...
class ConnectivityPlot(Plotting):
    ...
    def __init__(
        self, datastore, parameters, pnv_dsv=None, plot_file_name=None, fig_param=None
    ):
        Plotting.__init__(self, datastore, parameters, plot_file_name, fig_param)
        ...
        if pnv_dsv != None:
            ...
            z = queries.partition_analysis_results_by_parameters_query(
                pnv_dsv, parameter_list=["sheet_name"], excpt=True
            )
            for dsv in z:
        ...
        for conn in _connections:
            if (
                not self.parameters.reversed
                and conn.source_name == self.parameters.sheet_name
            ):
                ...
                idx = self.datastore.get_sheet_indexes(
                    conn.source_name, self.parameters.neuron
                )
                self.connected_neuron_position.append((z[0][idx], z[1][idx]))
                self.connections.append(conn)
            elif (
                self.parameters.reversed
                ...
                idx = self.datastore.get_sheet_indexes(
                    conn.target_name, self.parameters.neuron
                )
                self.connected_neuron_position.append((z[0][idx], z[1][idx]))
                self.connections.append(conn)
    ...
    def subplot(self, subplotspec):
        return LinePlot(
            function=self._ploter, length=self.length, shared_axis=True
        ).make_line_plot(subplotspec)
    ...
    def _ploter(self, idx, gs):
        ...
        if not self.parameters.reversed:
            index = self.datastore.get_sheet_indexes(
                self.connections[idx].source_name, self.parameters.neuron
            )
            ix = numpy.flatnonzero(
                numpy.array(self.connections[idx].weights)[:, 0] == index
            )
            ix = numpy.array(self.connections[idx].weights)[:, 1][ix].astype(int)
        else:
            index = self.datastore.get_sheet_indexes(
                self.connections[idx].target_name, self.parameters.neuron
            )
            ix = numpy.flatnonzero(
                numpy.array(self.connections[idx].weights)[:, 1] == index
            )
            ix = numpy.array(self.connections[idx].weights)[:, 0][ix].astype(int)

        assert all(
            numpy.array(self.connections[idx].weights)[:, 0]
            == numpy.array(self.connections[idx].delays)[:, 0]
        )
        assert all(
            numpy.array(self.connections[idx].weights)[:, 1]
            == numpy.array(self.connections[idx].delays)[:, 1]
        )
        ...
        if self.pnvs != None:
            for p in self.pnvs:
                if (
                    not self.parameters.reversed
                    and p.sheet_name == self.connections[idx].target_name
                ):
                    pnv.append(p)
                if (
                    self.parameters.reversed
        ...
        if pnv != []:
            (angle, mag) = circ_mean(
                numpy.array(
                    pnv.get_value_by_id(
                        self.datastore.get_sheet_ids(pnv.sheet_name, ix)
                    )
                ),
                weights=numpy.abs(w),
                high=pnv.period
            ...
            params["title"] = (
                str(self.connections[idx].proj_name)
                + "\n Mean: "
                + str(angle)
                + "\nNum conn/mean weight/total weight:"
                + str(len(w))
                + "/"
                + str(numpy.mean(w))
                + "/"
            ...
            if self.connections[idx].source_name == self.connections[idx].target_name:
                ...
                plots = [
                    (
                        "ConnectionsPlot",
                        ConnectionPlot(sx, sy, tx, ty, numpy.abs(w)),
                        gs,
                        params
                    )
                ]
...
class PerNeuronAnalogSignalScatterPlot(Plotting):
    ...
    def __init__(
        ...
        self.asls = queries.param_filter_query(
            datastore, name="AnalogSignalList"
        ).get_analysis_result()
        assert len(self.asls) == 2, (
            "PerNeuronAnalogSignalScatterPlot expects exactly two AnalogSignalList ADS"
            " in the datastore, found %d"
            % len(self.asls)
        )
    ...
    def _ploter(self, idx, gs):
        ...
        return [
            (
                "AnalogSignalScatterPlot",
                ScatterPlot(a.magnitude, b.magnitude),
                gs,
                {
                    "x_label": self.asls[0].y_axis_name,
                    "y_label": self.asls[1].y_axis_name,
                    "title": "Neuron id: %d" % self.parameters.neurons[idx]
                }
            )
        ]
...
class CorticalColumnRasterPlot(Plotting):
    ...
    def subplot(self, subplotspec):
        ...
        assert len(self.sheet_names) == len(self.labels) == len(self.colors), (
            "Parameter <sheet_names> , <labels> and <colors> have to have the same"
            " length or be empty lists."
        )

        if len(self.parameters.neurons) != 0:
            assert len(self.sheet_names) == len(
                self.parameters.neurons
            ), "Parameter <sheet_names> , <neurons> have to have the same length"

        return PerStimulusPlot(
            self.datastore, single_trial=True, function=self._ploter
        ).make_line_plot(subplotspec)
    ...
    def _ploter(self, dsv, gs):
        ...
        return [
            (
                "CorticalColumnRasterPlot",
                CorticalColumnSpikeRasterPlot(sp),
                gs,
...
class PlotTemporalTuningCurve(Plotting):
    ...
    def __init__(
        ...
        dsvs = queries.partition_analysis_results_by_parameters_query(
            self.datastore, parameter_list=["y_axis_name"], excpt=True
        )
        for dsv in dsvs:
            dsv = queries.param_filter_query(
                dsv,
            ...
            assert matching_parametrized_object_params(
                dsv.get_analysis_result(), params=["y_axis_name"]
            )
            ...
            self.st.append(st)
            dic = colapse_to_dictionary(
                [z.get_asl_by_id(self.parameters.neurons) for z in self.asls[-1]],
                st,
                self.parameters.parameter_name
            )
        ...
        if centering_pnv != None:
            ...
            period = st[0].getParams()[self.parameters.parameter_name].period
            assert period != None, (
                "ERROR: You asked for centering of tuning curves even though the domain"
                " over which it is measured is not periodic."
            )
            centers = centering_pnv.get_value_by_id(self.parameters.neurons)
            self.center_response_indexes.append(
                numpy.array(
                    [
                        numpy.argmin(circular_dist(par, centers[i], period))
                        for i in range(0, len(centers))
    ...
    def _ploter(self, idx, gs):
        ...
        for i, (dic, st, asl) in enumerate(zip(self.tc_dict, self.st, self.asls)):
                    ...
                    for j in range(0, len(self.parameters.neurons)):
                        if self.centered_response_indexes != None:
                            vv, p = self.center_tc(
                                ...
                                par,
                                period,
                                self.centered_response_indexes[i][j]
                            )
                        else:
                            vv = val[:, j]
                            p = par
            ...
            plots.append(
                (
                    "TuningCurve_" + asl[0].y_axis_name,
                    OrderedAnalogSignalListPlot(numpy.squeeze(ys), numpy.squeeze(xs)),
                    gs[i],
                    params
                )
            )
    ...
    def create_params(
        ...
        if units == munits.uS:
            ...
            params["colorbar_label"] = (
                y_axis_name + " (" + units.dimensionality.latex + ")"
            )
