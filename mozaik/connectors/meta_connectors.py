    ...
    def __init__(self, network, lgn_on, lgn_off, target, parameters, name):
        ...
        if self.parameters.or_map:
            ...
            coords_x = numpy.linspace(
                -t_size[0] / 2.0, t_size[0] / 2.0, numpy.shape(or_map)[0]
            )
        ...
        if self.parameters.phase_map:
            ...
            coords_y = numpy.linspace(
                -t_size[1] / 2.0, t_size[1] / 2.0, numpy.shape(phase_map)[1]
            )
            X, Y = numpy.meshgrid(coords_x, coords_y)
        ...
        for (j, neuron2) in enumerate(target.pop.all()):
            ...
            target.add_neuron_annotation(
                j, "LGNAfferentOrientation", orientation, protected=True
            )
            target.add_neuron_annotation(
                j, "LGNAfferentAspectRatio", aspect_ratio, protected=True
            )
            target.add_neuron_annotation(
                j, "LGNAfferentFrequency", frequency, protected=True
            )
