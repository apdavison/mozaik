...
class DataStoreView(ParametrizedObject):
    ...
    def get_sheet_ids(self, sheet_name, indexes=None):
        ...
        if isinstance(indexes, list) or isinstance(indexes, numpy.ndarray):
            ...
            raise ValueError(
                "indexes can be aither None or list or ndarray, %s was supplied instead"
                % (str(type(indexes)))
            )
    ...
    def get_sheet_parameters(self, sheet_name):
        ...
        """
        return eval(self.full_datastore.block.annotations["sheet_parameters"])[
            sheet_name
        ]
    ...
    def get_stimuli(self, null=False):
        ...
        """
        return [
            s.annotations["stimulus"] for s in self.block.segments if s.null == null
        ]
    ...
    def print_content(self, full_recordings=False, full_ADS=False):
        ...
        for st in [s.annotations["stimulus"] for s in self.block.segments]:
            d[MozaikParametrized.idd(st).name] = (
    ...
    def __add__(self, other):
        ...
        new_dsv.block.segments = list(
            set(self.block.segments) | set(other.block.segments)
        )
        new_dsv.analysis_results = list(
            set(self.analysis_results) | set(other.analysis_results)
        )
    ...
    def remove_ads_outside_of_dsv(self):
        if self.full_datastore != self:
            z = [
                ads
                for ads in self.full_datastore.analysis_results
                if ads not in self.analysis_results
            ]
            for ads in z:
                self.full_datastore.analysis_results.remove(ads)
...
class PickledDataStore(Hdf5DataStore):
    ...
    def save(self):
        ...
        f.close()
        print("check PickledDataStore save ", [a.annotations["source_ids"] for a in self.block.segments[0].analogsignals])
    ...
    def add_recording(self, segments, stimulus):
        ...
        self.stimulus_dict[str(stimulus)] = True
        print("check PickledDataStore add recording ", [a.annotations["source_ids"] for a in self.block.segments[0].analogsignals])
