...
def param_filter_query(dsv, ads_unique=False, rec_unique=False, **kwargs):
    ...
    if "sheet_name" in set(kwargs):
        if len(kwargs) == 1:
            ...
            kw = (
                kwargs["sheet_name"]
                if isinstance(kwargs["sheet_name"], list)
                else [kwargs["sheet_name"]]
            )
            seg_filtered = set(
                [s for s in dsv.block.segments if s.annotations["sheet_name"] in kw]
            )
        else:
            seg_filtered = set([])
    else:
    ...
    if st_kwargs != {}:
        seg_filtered_st = set(
            filter_query(seg_st, extra_data_list=dsv.block.segments, **st_kwargs)[1]
        )
        ads_filtered_st = set(
            filter_query(
                ads_st,
    ...
    if ads_unique and len(ads) != 1:
        raise ValueError(
            "Result was expected to have only single ADS, it contains %d" % len(ads)
        )

    if rec_unique and len(seg) != 1:
        raise ValueError(
            "Result was expected to have only single Segment, it contains %d" % len(seg)
        )
...
######################################################################################################################################
def partition_analysis_results_by_parameters_query(
...
def partition_analysis_results_by_stimulus_parameters_query(
    ...
    if excpt:
        ...
        parameter_list = set(st[0].getParams().keys()) - (
            set(parameter_list) | set(["name"])
        )
...
def equal_stimulus_type(dsv):
    ...
    return matching_parametrized_object_params(
        [MozaikParametrized.idd(s) for s in dsv.get_stimuli()], params=["name"]
    )
...
def equal_ads(dsv, params=None, except_params=None):
    ...
    """
    return matching_parametrized_object_params(
        dsv.analysis_results, params=params, except_params=except_params
    )
