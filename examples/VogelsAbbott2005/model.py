...
class VogelsAbbott(Model):
    ...
    required_parameters = ParameterSet(
        {
            "sheets": ParameterSet(
                {"exc_layer": ParameterSet, "inh_layer": ParameterSet,}
            )
        }
    )
    ...
    def __init__(self, sim, num_threads, parameters):
        ...
        UniformProbabilisticArborization(
            ...
            exc,
            inh,
            self.parameters.sheets.exc_layer.ExcInhConnection,
        ).connect()
        UniformProbabilisticArborization(
            self,
            "InhExcConnection",
            inh,
            exc,
            self.parameters.sheets.inh_layer.InhExcConnection,
        ).connect()
        UniformProbabilisticArborization(
            self,
            "InhInhConnection",
            inh,
            inh,
            self.parameters.sheets.inh_layer.InhInhConnection,
        ).connect()
